...
def load_graph(path, weighted=True, delimiter='\t', self_loop=False):
    graph = nx.Graph()
...
def create_argument_parser_main():
    ...
    parser.add_argument("-m", "--mean_function", help="'min', 'arithmetic', 'geometric', or 'harmonic'")
    return parser.parse_args()


class LSWLPlusCommunityDetection():
    minimum_improvement = 0.000001
    def __init__(self, graph, strength_type, merge_outliers, detect_overlap, nodes_to_ignore=set(), mean='geometric'):
        self.graph = graph
        ...
        if mean == 'min':
            self.mean_function = min
        elif mean == 'arithmetic':
            self.mean_function = statistics.mean
        elif mean == 'geometric':
            self.mean_function = statistics.geometric_mean
        elif mean == 'harmonic':
            self.mean_function = statistics.harmonic_mean
    ...
    def update_dicts_of_common_neighbors_info(self, node):
        ...
        for neighbor in self.graph.neighbors(node):
            if (neighbor in self.dict_common_neighbors[node]) is False:
                ...
                ###############
                # get weight of edge between node and neighbor
                weight1 = self.graph[node][neighbor]['weight']
                number_common_neighbors = 0 # initialize value to 0
                comm_neigh = nx.common_neighbors(self.graph, node, neighbor) # get common neighbors
                # iterate through all common neighbors
                for neigh in comm_neigh:
                    # get weight of edge between neighbor and common neighbor
                    weight2 = self.graph[neighbor][neigh]['weight']
                    # get weight of edge between node and common neighbour
                    weight3 = self.graph[node][neigh]['weight']
                    # "count" of common neighbors is the minimum of those three weights
                    weight = self.mean_function([weight1, weight2, weight3])
                    number_common_neighbors += weight
                ###############
...
if __name__ == "__main__":
    ...
    mean_function = args.mean_function if args.mean_function is not None else 'geometric'

    community_detector = LSWLPlusCommunityDetection(deepcopy(graph), strength_type, merge_outliers, detect_overlap, mean=mean_function)
